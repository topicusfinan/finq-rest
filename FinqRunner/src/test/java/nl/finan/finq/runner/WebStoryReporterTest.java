...
public class WebStoryReporterTest {
    ...
    public void testBeforeStory() throws Exception {
        ...
        PowerMockito.when(runningStoriesDao.find(101l)).thenReturn(runningStories);
        PowerMockito.when(storyDao.find(100l)).thenReturn(storyModel);
        when(reportService.createStoryLog(storyModel, runningStories)).thenReturn(storyLog);
        PowerMockito.when(storyLog.getId()).thenReturn(20l);
    ...
    public void testAfterStory() throws Exception {
        ...
        PowerMockito.when(runningStoriesDao.find(101l)).thenReturn(runningStories);
        PowerMockito.when(runningStories.getLogs()).thenReturn(Arrays.asList(storyLog));
    ...
    public void testBeforeScenario() throws Exception {
        ...
        PowerMockito.when(scenario.getTitle()).thenReturn("Scenario Title");
        when(reportService.findStoryLog(20l)).thenReturn(storyLog);
        PowerMockito.when(storyLog.getStory()).thenReturn(story);
        PowerMockito.when(story.getScenarios()).thenReturn(scenarios);
        PowerMockito.when(scenarioLog.getId()).thenReturn(100l);
    ...
    public void testFailed() throws Exception {
        ...
        PowerMockito.when(throwable.getMessage()).thenReturn("Assertion Failed!");
        PowerMockito.when(runningStoriesDao.find(101l)).thenReturn(runningStories);
        PowerMockito.when(runningStories.getLogs()).thenReturn(Arrays.asList(storyLog));
